...
python manage.py test gym          # run all 91 tests
...
- Components (`src/components/`): Sidebar, MobileTopBar, FlashMessages, Navbar, CoachWidget, ExercisePicker (search input + body-part filter pills with counts + sticky "N selected" bar with removable chips; shared by `PresetForm` and `DayCreate`'s custom-workout flow — extend this component rather than re-duplicating picker markup in either page).
...
- `POST /api/coach/` takes `{"messages": [{"role": "user"|"assistant", "content": str}, ...]}` (the client's whole visible conversation), truncates to the last `COACH_MAX_HISTORY = 20` and each message to `COACH_MAX_MESSAGE_LEN = 2000` chars server-side (never trust the client for bounds), prepends a system prompt built by `_coach_workout_summary(user)` (this week's workout count, days since last workout, and the last 3 workouts' preset + exercise names **with each exercise's sets** as `weight_unit x reps`, e.g. `Bench Press: 60kgx8, 62.5kgx6` — sorted by `set_number`, weights trimmed via `_fmt_weight`, so Coach can answer specific questions like "how did I do today?"), and proxies to OpenRouter's `/chat/completions` with `requests`. Returns `{"reply": str}`.
...
- **Model fallback chain** (`OPENROUTER_FALLBACK_MODELS`, default Gemma 4 26B + Llama 3.3 70B, both `:free`): free pools saturate *per-model*, so the payload includes OpenRouter's native `models` array (primary first) and their router transparently serves from the first pool with capacity instead of 429ing. This is the main defense; the retry loop above only matters when every model in the chain is busy at once. Set the env var to `""` to disable (payload then sends `model` only).
- **OpenRouter's Activity dashboard only counts successful generations** — 429'd calls never appear there. "Coach is busy" + 0 requests on the dashboard does NOT mean requests aren't reaching OpenRouter; a 429 response *is* OpenRouter answering. (A misconfigured key shows "couldn't be reached"/502 instead, since OpenRouter returns 401 for those.)
...
- **Flex children need explicit `min-width: 0` to respect the viewport**: `.main-content` is a flex item, and a flex item's `min-width` defaults to `auto` — meaning it will never shrink narrower than its own content's natural width. Any no-wrap horizontal content inside it (e.g. `ExercisePicker`'s `.picker-filters` pill row) silently forces the *entire page* wider than the viewport on mobile; the browser zooms the whole layout out instead of wrapping, which reads as "the site is broken on my phone," not an obvious CSS bug. Fixed once via `min-width: 0` on `.main-content`, but any new horizontally-scrolling or no-wrap UI on a page should be checked at 375px width before shipping — a passing build and test suite won't catch this.
